...
const ProjectCard = ({ projects }) => {
    return (<>
        <div className="flex gap-[2em] justify-around flex-wrap">
            {
                projects.map((e,index) => {
                    return (
                        <div className="card bg-sky-100 shadow-lg rounded-lg overflow-hidden transform transition-transform hover:scale-110  " key={index}>
                            {/* <Image className="rounded-full border-2 border-black p-2 w-24 h-24 mx-auto mt-4" src={e.logo} width={10} height={10} alt="Logo" /> */}
                            <div className="p-6 max-w-[30em]">
                                ...
                                <div className="flex justify-around items-center flex-wrap"> 
                                {
                                    e.tags.map((xe, index)=>{
                                        return(                                            
                                            <div className="rounded-full bg-sky-200 p-2 text-black mt-3" key={index}>{xe}</div>
                                        )
                                    })
                                }
                                </div>
